...
## Destination Identity Cards

On 15 August 2026, `/links?search=Bangladesh` was checked at **1280×720** and **375×812** after adding destination identity cards. Where a favicon is available, a same-origin, cached logo is shown; unavailable sources retain a clear two-character name monogram instead of an empty or broken image. The grid remains three columns on desktop and one column on phone, with recognisable marks, titles, descriptions, tags, and external-link affordances kept legible. The browser receives icons only from ToolsHUB’s same-origin route, which forwards only a validated hostname to the icon provider and does not expose the visitor’s referrer or the source URL.
