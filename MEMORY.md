...
- The genuine-playable catalogue has grown to 16 items. The next staged release is Breakout, Flappy Flight, Fruit Slice, and Space Dodge. The public grid filters `playable: true`, so the release must change data and dispatch mapping together.
- An original arcade direction reference for this batch is available at `/manus-storage/tools-hub-arcade-direction_e4b157b4.png`; it must be used as a decorative visual reference rather than copied game art.
