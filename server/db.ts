...
export async function saveUserSettings(userId: number, values: { language: "en" | "bn" | "hi" | "ur" | "ar" | "es" | "fr" | "de"; appearance: string; customColors?: unknown; playfulEffects?: { cat: boolean; trail: boolean; buddy: boolean } }) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const existing = await getUserSettings(userId); const previousPreferences = typeof existing?.aiPreferences === "object" && existing.aiPreferences !== null && !Array.isArray(existing.aiPreferences) ? existing.aiPreferences as Record<string, unknown> : {}; const aiPreferences = values.playfulEffects ? { ...previousPreferences, playfulEffects: values.playfulEffects } : previousPreferences; await db.insert(userSettings).values({ userId, language: values.language, appearance: values.appearance, customColors: values.customColors, aiPreferences }).onDuplicateKeyUpdate({ set: { language: values.language, appearance: values.appearance, customColors: values.customColors, aiPreferences } }); return getUserSettings(userId); }
