import { ArrowRight, BookOpenText, Braces, FileImage, Gamepad2, LockKeyhole, Search, Sparkles, WandSparkles } from "lucide-react";
...
const homeCopy = {
  en: { quick: "Start with a common task", quickCopy: "Open a focused workspace in one click", image: "Edit an image", pdf: "Work with a PDF", json: "Format JSON" },
  bn: { quick: "জনপ্রিয় কাজ দিয়ে শুরু করুন", quickCopy: "এক ক্লিকে নির্দিষ্ট ওয়ার্কস্পেস খুলুন", image: "ছবি সম্পাদনা", pdf: "PDF নিয়ে কাজ", json: "JSON ফরম্যাট" },
  hi: { quick: "सामान्य कार्य से शुरू करें", quickCopy: "एक क्लिक में केंद्रित कार्यस्थान खोलें", image: "छवि संपादित करें", pdf: "PDF पर काम करें", json: "JSON फ़ॉर्मेट करें" },
  ur: { quick: "عام کام سے آغاز کریں", quickCopy: "ایک کلک میں مربوط ورک اسپیس کھولیں", image: "تصویر میں ترمیم", pdf: "PDF کے ساتھ کام", json: "JSON فارمیٹ" },
  ar: { quick: "ابدأ بمهمة شائعة", quickCopy: "افتح مساحة عمل مركزة بنقرة واحدة", image: "حرر صورة", pdf: "اعمل مع PDF", json: "نسّق JSON" },
  es: { quick: "Empieza con una tarea habitual", quickCopy: "Abre un espacio de trabajo con un clic", image: "Editar una imagen", pdf: "Trabajar con un PDF", json: "Formatear JSON" },
  fr: { quick: "Commencez par une tâche courante", quickCopy: "Ouvrez un espace de travail ciblé en un clic", image: "Modifier une image", pdf: "Travailler avec un PDF", json: "Formater JSON" },
  de: { quick: "Mit einer häufigen Aufgabe beginnen", quickCopy: "Mit einem Klick einen fokussierten Arbeitsbereich öffnen", image: "Bild bearbeiten", pdf: "Mit einem PDF arbeiten", json: "JSON formatieren" },
} as const;

export default function Home() {
  const { language, t } = useSettings();
  const quick = homeCopy[language];
  ...
  return <>
    <section className="hero"><div className="hero-mesh" /><div className="site-frame hero-grid"><div className="hero-copy"><p className="eyebrow"><Sparkles size={15} />{t("hero.eyebrow")}</p><h1>{t("hero.title")}</h1><p className="hero-description">{t("hero.copy")}</p><div className="hero-actions"><Link href="/tools" className="primary-cta">{t("hero.explore")} <ArrowRight size={18} /></Link><Link href="/games" className="secondary-cta"><Gamepad2 size={18} />{t("hero.play")}</Link><Link href="/links" className="secondary-cta library-cta"><BookOpenText size={18} />{t("nav.links")} <b>2,173</b></Link></div><div className="hero-quickstart"><div><strong>{quick.quick}</strong><span>{quick.quickCopy}</span></div><div className="quickstart-actions"><Link href="/tools/image-crop"><FileImage size={15} />{quick.image}</Link><Link href="/tools/pdf-to-image"><BookOpenText size={15} />{quick.pdf}</Link><Link href="/tools/json-formatter"><Braces size={15} />{quick.json}</Link></div></div><div className="trust-row"><span><LockKeyhole size={15} />{t("hero.private")}</span><span><b>244+</b> {t("nav.tools")}</span><span><b>40+</b> {t("nav.games")}</span></div></div><div className="hero-visual" aria-hidden="true"><div className="orbit orbit-one" /><div className="orbit orbit-two" /><div className="visual-card visual-search"><Search /><span>JSON Formatter</span><i>↵</i></div><div className="visual-card visual-palette"><span>Color Lab</span><div><i /><i /><i /><i /></div></div><div className="visual-card visual-ai"><WandSparkles /><span>AI</span></div><div className="hero-core"><span>T</span><small>HUB</small></div></div></div></section>
    <section className="site-frame directory-ribbon-shell"><Link href="/links" className="directory-ribbon"><span className="directory-ribbon-icon"><BookOpenText size={24} /></span><span className="directory-ribbon-copy"><span className="eyebrow">{t("links.eyebrow")}</span><strong>{t("links.title")}</strong><small>{t("links.copy")}</small></span><span className="directory-ribbon-meta"><b>2,173</b><small>{t("links.results")}</small><ArrowRight size={19} /></span></Link></section>
    <section className="site-frame section-block"><div className="section-title-row"><div><p className="eyebrow">01 / {t("home.discover")}</p><h2>{t("home.categories")}</h2><p>{t("home.categoriesCopy")}</p></div><Link href="/tools" className="text-link">{t("common.viewAll")} <ArrowRight size={16} /></Link></div><div className="category-grid">{toolCategories.map((category) => { const Icon = category.icon; return <Link href={`/tools?category=${category.id}`} className={`category-card tone-${category.color}`} key={category.id}><span className="category-index">{String(category.count).padStart(2, "0")}</span><span className="category-icon"><Icon size={22} /></span><div><h3>{t(`category.${category.id}`)}</h3><p>{category.count}+ {t("tools.found")}</p></div><ArrowRight size={17} /></Link>; })}</div></section>
