...
export function ThemePanel() {
  ...
  const parseImport = () => { try { const parsed = JSON.parse(importValue); const next = parsed.tokens ?? parsed; if (!next || !labels.every(({ key }) => /^#[0-9a-f]{6}$/i.test(next[key]))) throw new Error(); setTokens(next); setName(typeof parsed.name === "string" && parsed.name.trim() ? parsed.name.trim().slice(0, 60) : t("appearance.importedName")); setNotice(t("appearance.imported")); } catch { setNotice(t("appearance.invalid")); } };
